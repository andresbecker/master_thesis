"""MPP_dataset_no_pert_no_mask dataset."""
...
class MppDataset_no_pert_no_mask(tfds.core.GeneratorBasedBuilder):
	"""DatasetBuilder for _no_pert_no_mask dataset."""
	...
	def _info(self) -> tfds.core.DatasetInfo:
		...
		# images data type
		img_dtype = getattr(tf, pp_param['images_dtype'])

		# set target shape accordinglly with the aggregate_output
		self.aggregate_output = pp_param['aggregate_output']
		if (self.aggregate_output == 'avg_no_mask'):
			target_shape = (1,2)
		else:
			target_shape = (1,)
		...
		return tfds.core.DatasetInfo(
			...
			features=tfds.features.FeaturesDict({
				...
				'target': tfds.features.Tensor(shape=target_shape, dtype=tf.float64),
				}),
	...
	def _generate_examples(self, images_path):
		...
		for fn in file_names:
			...
			if (self.aggregate_output == 'avg_no_mask'):
				cell_target = cell['targets'][self.output_id].reshape((1,2))
			else:
