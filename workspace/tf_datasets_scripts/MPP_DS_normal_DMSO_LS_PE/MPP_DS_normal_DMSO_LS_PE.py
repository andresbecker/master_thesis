"""MPP_DS_normal_DMSO_LS_PE dataset."""
...
class MPP_DS_normal_DMSO_LS_PE(tfds.core.GeneratorBasedBuilder):
	"""DatasetBuilder for MPP_DS_normal_DMSO_LS_PE dataset."""
	...
	def _apply_preprocessing(self, img, cell_mask):
		...
		"""
		percentile = self.tfds_param['percentile']
		median = int(percentile / 2)

		# Apply rescaling if selected
		if self.tfds_param['apply_linear_scaling']:
			img /= self.train_params['P_'+str(percentile)]

		# Apply median extraction if selected
		if self.tfds_param['apply_median_extraction']:
			img[cell_mask] -= self.train_params['P_'+str(median)]
	...
	def _get_train_per_channel_parameters(self, percentile):
		...
		train_params['P_'+str(percentile)] = np.percentile(train_pixels, percentile, axis=0).astype(np.float32)

		# if lin scaling was selected, then apply it before getting the median
		if self.tfds_param['apply_linear_scaling']:
			train_pixels /= train_params['P_'+str(percentile)]

		median = int(percentile/2)
		train_params['P_'+str(median)] = np.percentile(train_pixels, median, axis=0).astype(np.float32)
	...
	def _create_metadata(self):
		...
		percentil_col_name = 'train_P_'+str(percentile)
		temp_df[percentil_col_name] = self.train_params['P_'+str(percentile)]
		median_col_name = 'train_P_'+str(median)+'_after_LS'
		temp_df[median_col_name] = self.train_params['P_'+str(median)]
		temp_df['type'] = 'input'
		...
		temp_dict = {'original_channel_id': 'NaN', 'TFDS_channel_id': len(self.input_ids), 'name':'cell_mask', percentil_col_name:'NaN', median_col_name:'NaN', 'type':'NaN'}
		self.channels = self.channels.append(temp_dict, ignore_index=True)

		# save info about output channel
		temp_dict = {'original_channel_id': self.output_id, 'TFDS_channel_id': 'NaN', 'name':(self.tfds_param['output_channels'])[0], percentil_col_name:'NaN', median_col_name:'NaN', 'type':'output'}
		self.channels = self.channels.append(temp_dict, ignore_index=True)
