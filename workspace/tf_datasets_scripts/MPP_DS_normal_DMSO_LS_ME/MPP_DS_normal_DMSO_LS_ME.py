"""MPP_DS_normal_DMSO_LS_ME dataset."""
...
class MPP_DS_normal_DMSO_LS_ME(tfds.core.GeneratorBasedBuilder):
	"""DatasetBuilder for MPP_DS_normal_DMSO_LS_ME dataset."""
	...
	def _info(self) -> tfds.core.DatasetInfo:
		...
		# Get percentiles for input channels and train partition
		self.input_channel_percentiles, self.channels_mean = self._get_per_channel_percentiles_and_mean(self.tfds_param['percentile'])
	...
	def _apply_clipping(self, img):
		"""
		Apply cliping to all partitions using the given percentile_vec
		"""
		for c in range(self.input_channel_percentiles.shape[0]):
			temp_mask = (img[:,:,c] > self.input_channel_percentiles[c])
			img[:,:,c][temp_mask] = self.input_channel_percentiles[c]
	...
	def _get_channels_mean(self, pixels, percentiles):
		"""
		Returns channels mean. If clipping and/or sclaing were selected, they need to be applyed before getting the channel mean.
		"""

		# if clipping was selected, then apply it before pixels before getting the per-channel mean
		if self.tfds_param['apply_clipping']:
			for c in range(pixels.shape[-1]):
				temp_mask = (pixels[:,c] > percentiles[c])
				pixels[:,c][temp_mask] = percentiles[c]

		# if lin scaling was selected, then apply it before pixels before getting the per-channel mean
		if self.tfds_param['apply_linear_scaling']:
			pixels /= percentiles

		return pixels.mean(axis=0)

	def _get_per_channel_percentiles_and_mean(self, percentile):
		"""
		Get the per-channnel percentile and mean from the train set.
		"""
		...
		channel_percentiles = np.percentile(train_pixels, percentile, axis=0).astype(np.float32)

		channels_mean = self._get_channels_mean(train_pixels, channel_percentiles)

		return channel_percentiles, channels_mean
	...
	def _create_metadata(self):
		...
		temp_df[percentil_col_name] = self.input_channel_percentiles
		mean_col_name = 'train_mean_after_clipping_and_LS'
		temp_df[mean_col_name] = self.channels_mean
		temp_df['type'] = 'input'
		...
		temp_dict = {'original_channel_id': 'NaN', 'TFDS_channel_id': len(self.input_ids), 'name':'cell_mask', percentil_col_name:'NaN', mean_col_name:'NaN', 'type':'NaN'}
		self.channels = self.channels.append(temp_dict, ignore_index=True)

		# save info about output channel
		temp_dict = {'original_channel_id': self.output_id, 'TFDS_channel_id': 'NaN', 'name':(self.tfds_param['output_channels'])[0], percentil_col_name:'NaN', mean_col_name:'NaN', 'type':'output'}
		self.channels = self.channels.append(temp_dict, ignore_index=True)
