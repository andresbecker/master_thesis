...
class MPP_dataset_test1(tfds.core.GeneratorBasedBuilder):
	"""DatasetBuilder for MPP_dataset_test1 dataset."""
	VERSION = tfds.core.Version('1.0.0')
	RELEASE_NOTES = {
		'1.0.0': 'First Release. Only normal, DMSO (perturbations). This TFDS keeps an equal number of cells in cell cycle G1, S, G2 across the train, val and test set.',
		}
	...
	def _add_train_val_test_split_to_metadata(self, filtered_metadata):
		...
		# Get the cell cycle with the minimum number of cells
		min_cc_size = np.Inf
		for cc in self.tfds_param['cell_cycles']:
		    ...
		    if cc_size < min_cc_size:
		        min_cc_size = cc_size
		        min_cc_name = cc
		print('Smallest convination:\n{}, {}'.format(min_cc_name, min_cc_size))
		...
		for cc in self.tfds_param['cell_cycles']:
			# Create mask that contains the cells with the corresponding cell_cycle
			mask = (filtered_metadata.cell_cycle == cc)
			cell_ids = filtered_metadata[mask].mapobject_id_cell.values

			# Create split sizes (same for all the cell cycles)
			n_train = int(min_cc_size * self.tfds_param['train_frac'])
			n_val = int(min_cc_size * self.tfds_param['val_frac'])
			n_test = min_cc_size - n_train - n_val
			print('For cell cycle {};\n\tNumber of cells in train set: {}, val set: {}, test_set: {}\n\tTotal number of cells: {}\n'.format(cc, n_train, n_val, n_test, n_train+n_val+n_test))
