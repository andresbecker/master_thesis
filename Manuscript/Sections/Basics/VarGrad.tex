...
\begin{figure}[!ht]
  ...
  \end{subfigure}
  \caption{Comparative between a cell image and the different attribution methods. All the figures show the same 3 channels taken from a cell image. In \subref{fig:vg:cell_img}), the cell image, no attribution method. In \subref{fig:vg:img_gradients}), the gradient of the model with respect to the input image. In \subref{fig:vg:img_IG}), the \gls{ig} with respect to the input image. In \subref{fig:vg:img_VG_IG}), the composition of \gls{vg} with \gls{ig} with respect to the input image.}
